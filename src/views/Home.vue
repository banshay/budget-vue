<template>
  <div class="h-screen flex flex-col">
    ...
    <div class="mx-auto mt-10 flex-grow">
      <p class="text-gray-200 text-7xl">{{ balance > 0 ? '+' : '' }}{{ balance }}</p>
    </div>
    <div class="flex justify-between items-end">
      <div class="w-3/5">
        <div class="m-10 text-gray-200 w-full">
          ...
          <div class="flex">
            <div class="ml-2 text-left">yesterday</div>
            <div class="mx-12 text-left">Food</div>
            <div class="text-left">-12.95</div>
          </div>
        </div>
      </div>
      <div class="m-10 w-16 h-16 bg-lime-500 rounded-full">
        <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <a href="#" @click="saveToken">Save Token</a>
    </div>
  </div>
</template>
...
import useMoney from '@/composition/money';
import {token} from '@/firebase';

export default defineComponent({
  name: 'Home',
  setup() {
    const {loadBalance} = useMoney()
    ...
    const saveToken = () => {
      token().then(token => sessionStorage.setItem('token', token || ''))
    }

    return {balance, saveToken}
  },
})
</script>
